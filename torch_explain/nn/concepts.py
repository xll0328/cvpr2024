...
class ConceptReasoningLayer(torch.nn.Module):
    ...
    def explain(self, x, c, mode, concept_names=None, class_names=None, filter_attn=None):
        ...
        if concept_names is None:
            concept_names = [f'c_{i}' for i in range(c.shape[0])]
        ...
        sign_attn_mask=torch.unsqueeze(sign_attn_mask, dim=0)
        filter_attn_mask=torch.unsqueeze(filter_attn_mask, dim=0)
        explanations = []
        ...
        #for sample_idx in range(len(x))
            ...
            if 1==0:
                ...
                for target_class in active_classes:
                    ...
                    minterm = []
                    #print(concept_names)
                    #print(sign_attn_mask.size())
                    #print(filter_attn_mask.size())
