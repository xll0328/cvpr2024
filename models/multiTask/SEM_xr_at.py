...
class SEM(nn.Module):
    def __init__(self, args):
        ...
        #self.post_fusion_layer_1 = nn.Linear(args.text_out + args.text_out + args.audio_out +123*self.embedding_size, args.post_fusion_dim)
        self.post_fusion_layer_1 = nn.Linear(args.video_out +82*self.embedding_size, args.post_fusion_dim+args.audio_out)
    ...
    def forward(self, text, audio, video):
        ...
        #c_pred = audio_concept_pred
        #print(c_emb.size())
        #print(c_pred.size()) 
        c_emb = torch.cat((audio_concept_emb, text_concept_emb), dim=1)
        a,b,c = c_emb.shape
        cc_all = c_emb.view(a,b*c)
        
        # classifier-fusion
        #fusion_h = torch.cat([text, audio, video], dim=-1)
        fusion_h = torch.cat([audio, text], dim=-1)
